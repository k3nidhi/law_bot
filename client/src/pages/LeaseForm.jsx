...
export default function LegalDocAssistant() {
    ...
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2 p-8 gap-8 bg-white text-gray-800 font-serif">
      ...
      <div className="space-y-4 border rounded-xl p-6 shadow-sm">
        ...
        <div className="pt-4 text-center">
          <Button disabled className="w-full bg-gray-400 cursor-not-allowed">Download PDF (Coming Soon)</Button>
        </div>
      </div>
      ...
      <div className="border rounded-xl p-6 shadow-sm whitespace-pre-wrap overflow-y-auto">
        ...
        <p className="leading-relaxed">
          This Deed of Lease is made between <strong>{lessorName || "[Lessor]"}</strong> of {lessorAddress || "[Address]"} and <strong>{lesseeName || "[Lessee]"}</strong> of {lesseeAddress || "[Address]"}.

          {'\n\n'}The Lessor hereby leases the property at {propertyAddress || "[Property Address]"} to the Lessee for a term of {leaseYears || "[X]"} years, commencing from {startDate || "[Start Date]"}, for a monthly ground rent of Rs. {monthlyRent || "[Amount]"}, payable on or before the 5th of every month.

          {'\n\n'}If the rent is unpaid, the Lessee shall pay interest at {interestRate || "[Rate]"}% per annum. The Lessee shall also pay municipal taxes of Rs. {municipalTax || "[Amount]"} annually.

          {'\n\n'}The Lessee may construct, alter, or renovate structures on the property as permitted by law, and shall maintain the premises in tenantable condition.

          {'\n\n'}This agreement shall be governed by mutual terms and termination clauses as per the standard format.
